...
*/
function LBWhereToFilters(model, where, statement) {
    if(!where) where = {};
    ...
    //     // cant use a UDF stream filter and filters
    //     delete statement.filters;
    //     statement.aggregationUDF = {
    //         module:"operators",
    //         funcname: "operators"
    //     };
    // }
    ...
    if (self.db) {
        ...
        self.db.connect(function (response) {
            ...
            if (response.code === 0) {
                ...
                self.db.udfRegister(path.resolve(__dirname, "../UDFs/operators.lua"), function(err) {
                    if(callback) {
                        if (err.code === 0)
...
Aerospike.prototype.all = function all(model, filter, callback) {
    ...
    if(filter.where && filter.where.id !== undefined) {
        ...
        var statement = LBWhereToFilters.call(this, model, filter.where, {
            concurrent: true,
            nobins: false,
            filters: []
        });
